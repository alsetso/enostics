...
CREATE TABLE IF NOT EXISTS public.enostics_data (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    endpoint_id UUID REFERENCES public.enostics_endpoints(id) ON DELETE CASCADE NOT NULL,
    data JSONB NOT NULL,
    source_ip TEXT,
    headers JSONB,
...
BEGIN
    ...
    -- Create initial URL path
    url_path_value := LOWER(REGEXP_REPLACE(username_value, '[^a-zA-Z0-9]', '', 'g'));
...
ALTER TABLE public.enostics_data ENABLE ROW LEVEL SECURITY;
...
-- Create RLS policies for enostics_endpoints
CREATE POLICY "Users can view own endpoints" ON public.enostics_endpoints
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can update own endpoints" ON public.enostics_endpoints
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own endpoints" ON public.enostics_endpoints
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete own endpoints" ON public.enostics_endpoints
    FOR DELETE USING (auth.uid() = user_id);

-- Create RLS policies for enostics_data
CREATE POLICY "Users can view own endpoint data" ON public.enostics_data
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.enostics_endpoints 
            WHERE id = endpoint_id AND user_id = auth.uid()
        )
    );

CREATE POLICY "Anyone can insert data to endpoints" ON public.enostics_data
...
CREATE INDEX IF NOT EXISTS idx_enostics_endpoints_url_path ON public.enostics_endpoints(url_path);
CREATE INDEX IF NOT EXISTS idx_enostics_data_endpoint_id ON public.enostics_data(endpoint_id);
CREATE INDEX IF NOT EXISTS idx_enostics_data_processed_at ON public.enostics_data(processed_at);
